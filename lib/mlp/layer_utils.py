...
class fc(object):
    ...
    def forward(self, feat):
        ...
        output = np.add(np.dot(feat,self.params[self.w_name]), self.params[self.b_name])
...
class gelu(object):
    ...
    def backward(self, dprev):
        ...
        # Store the output gradients in the variable dfeat provided above.          #
        #############################################################################
        deri =  0.5*np.tanh(0.0356774*feat**3 + 0.797885*feat) + 0.5 + (0.0535161*feat**3 + 0.398942*feat) * 1/np.cosh(0.0356774*feat**3 + 0.797885*feat)**2
...
class dropout(object):
    ...
    def forward(self, feat, is_training=True, seed=None):
        ...
        if is_training and self.keep_prob:
            kept = (self.rng.rand(feat.shape[0], feat.shape[1]) <= self.keep_prob).astype(float)*(1/self.keep_prob)
        else:
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        for each in label:
            sum += np.log(logit[i][each])*-1
            i+=1
        if self.size_average:
            loss = sum/feat.shape[0]
        else:
            loss = sum
    ...
    def backward(self):
        ...
        for each in label:
            ...
            i+=1
        if self.size_average:
            dlogit = logit/self.label.shape[0]
        else:
            dlogit = logit
